...
const Login = () => {
  ...
  async function getTransferKeys(email) {
    try {
      const q = query(
        collection(db, "transfers"),
        where("emailOrMobile", "==", email),
        limit(1),
      );
  ...
  //     const userSnap = await getDoc(userRef);
  //     const usernameifexists = await getTransferKeys(user.email);
  ...
  //         uid: user.uid,
  //         email: user.email,
  //         name: usernameifexists ? usernameifexists.name : "New User", // Default name (can be updated later)
  ...
  //     createPasskey(); // Call createPasskey after successful login
  ...
  //     const transferData = await getTransferKeys(email);
  ...
  const handleTransferredUser = async (email, password) => {
    const transferData = await getTransferKeys(email);
    ...
    // OPTIONAL:
    // Delete the transfer document now that it's been claimed.
    // await deleteDoc(doc(db, "transfers", transferData.id));

    return true;
  };

  const handleExistingUser = async (userDoc, password) => {
    ...
    try {
      ...
      if (
        ...
        // Legacy user without a Firebase Auth account
  ...
  const ensureAuthAccount = async (email, password) => {
    const methods = await fetchSignInMethodsForEmail(auth, email);

    if (methods.includes("password")) {
      // Auth account already exists, so just sign in.
      return await signInWithEmailAndPassword(auth, email, password);
    }

    if (methods.length > 0) {
      // Email exists in Auth, but not with password login.
      // Let sign-in fail naturally instead of trying to create.
      return await signInWithEmailAndPassword(auth, email, password);
    }

    // No Auth account yet, so create one.
    try {
      return await createUserWithEmailAndPassword(auth, email, password);
    } catch (err) {
      // Small fallback in case another request created it in the meantime.
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!snapshot.empty) {
        ...
        // Legacy Firestore password check
        if (userData.password && userData.password !== password) {
          toast.error("🔑 Incorrect password.");
          return;
        }

        // Keep name synced from transfers if needed
        if (transferData?.name && transferData.name !== userData.name) {
      ...
      // Make sure Firebase Auth exists and the user is signed in
      const authCredential = await ensureAuthAccount(email, password);
      const user = authCredential.user;

      // If no Firestore user existed yet, create one now
      if (snapshot.empty) {
        const userRef = doc(db, "users", user.uid);
        ...
        // Keep Firestore uid aligned with Auth uid if needed
      ...
      // Optional: once migration is confirmed working, remove legacy password
      ...
      let alreadyHasPasskeyforEmail = localStorage.getItem(`passkeyCreated_${email}`) === "true";
      if (alreadyHasPasskeyforEmail === false || alreadyHasPasskeyforEmail === null) {
        // Handle master user specific logic
        await createPasskey();
        localStorage.setItem(`passkeyCreated_${email}`, "true");
      }
      navigate(redirectPath);
    } catch (err) {
      console.error(err);
  ...
  //     if (!snapshot.empty) {
  //       // Existing user
  //       await handleExistingUser(snapshot.docs[0], password);
  //     } else {
  //       // First-time transferred user
  //       const success = await handleTransferredUser(email, password);
    ...
    <>
      <div className="flex flex-col items-center justify-center text-black bg-white my-14 mx-5">
        ...
        <div className="w-full max-w-md mb-12">
          ...
          />
          {(emailRegex.test(email) || email == "Usegen@ticket.com") && (
        ...
        <div className="w-full max-w-md">
          <button
            className={`mb-4 ${(emailRegex.test(email) || email == "Usegen@ticket.com") && !loading && password !== "" ? "bg-customBlue text-white" : "text-neutral-400 bg-neutral-100"} text-[0.9375rem] w-full max-w-md h-11 rounded-[0.2188rem] font-bold transition-all duration-300 ease-in-out flex items-center justify-center`}
            onClick={
